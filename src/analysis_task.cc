...
#define proton 2212 //int particle ID
...
void AnalysisTask::Init(std::map<std::string, void *> &branch_map) {
  ...
  MATCHED_PTvY_= new TH2F("Matched_Y_vs_pT","Loss;Y;p_{T}", 250, 0., 2.5, 250, 0., 2.);
...
void AnalysisTask::Exec() {
  ...
  for (int i = 0; i < n_tracks; ++i) { // loop over all tracks if current event
    ...
    int matched_track = track_matching_->GetMatchDirect(i);
    auto sim_track = sim_tracks_->GetChannel(matched_track);
    int sim_pid = sim_track.GetPid();
    ...
    p = p / charge;
    if (beta < 1.0) mass = p * sqrt(1.0 - beta * beta) / beta; //c = 1
    else mass = p * sqrt(beta * beta - 1.0) / beta;
    /*
    ...
    */
    ...
    bool is_prim = sim_track.GetField<bool>(0); // is_primary
    ...
    if(pid == particle){
      ...
      if (is_prim) PID_PRIM_->Fill(rapidity, pT);
      else PID_SEC_->Fill(rapidity, pT);

      if(sim_pid == particle){
        if (is_prim) PDG_PRIM_->Fill(rapidity, pT);
        else  PDG_SEC_->Fill(rapidity, pT);
      }
      else MISMATCH_->Fill(rapidity, pT);
      
      //PTvRAPIDITY_->Fill(rapidity, pT);
      PTvPSEUDORAPIDITY_->Fill(eta, pT);

      if(sim_pid == particle && !is_prim)
      MATCHED_PTvY_->Fill(sim_rapidity, sim_pT);

    //mass2_distribution_->Fill(mass*mass);
    //mass2_distribution_branch_->Fill(m2);
    //PvTOF_->Fill(p, tof);
    //PvPathLen_->Fill(p, path_len);
    }
    else {
       if (sim_pid == particle) LOSS_->Fill(rapidity, pT);
    }

    if(sim_pid == particle){
      GEN_->Fill(sim_rapidity, sim_pT);
      if (is_prim) GEN_PRIM_->Fill(sim_rapidity, sim_pT);
      else GEN_SEC_->Fill(sim_rapidity, sim_pT);
    }
